(function () {'use strict';angular.module('myapp').factory('preLoaderTemplate', ['$templateCache',  function ($templateCache) {  'use strict';
  ...
  $templateCache.put('/wwwroot/app/propose/index.html',
    ...
    "\n" +
    "    </ol></script><div ng-include=\"'breadcrumbProposer.html'\"></div><h1>Proposer vos biens</h1><div><button type=\"button\" class=\"btn btn-default pull-right\" ng-click=\"vm.navAdd()\">Ajouter un bien</button></div><div ng-if=\"vm.Biens.length >0\"><h2>Liste des biens</h2><table class=\"table\" ng-repeat=\"bien in vm.Biens\"><thead><tr><th>Titre</th><th>Date</th><th>Action</th></tr></thead><tbody><tr><td><span>Loué du {{bien.DateDebut | date:'dd/MM/yyyy' }} au {{bien.DateFin | date:'dd/MM/yyyy' }} : {{bien.Frequence}} de {{bien.DateDebut | date:'HH:mm' }} au {{bien.DateFin | date:'HH:mm' }}</span></td></tr><tr class=\"height-20\"><td><span ng-bind=\"bien.Titre\"></span></td><td><span>{{bien.DateDebut | date:'dd/MM/yyyy' }}</span> <span>{{bien.DateFin | date:'dd/MM/yyyy' }}</span></td><td><button ng-click=\"vm.remove(bien)\" type=\"button\" class=\"btn btn-danger\" uib-popover=\"Supprimer\" popover-trigger=\"mouseenter\"><span class=\"glyphicon glyphicon-remove\"></button></td></tr></tbody></table><div ng-if=\"vm.Biens.length <=0\" class=\"oeup-empty\"><p>Aucun bien déclaré.</p></div><div><button type=\"button\" class=\"btn btn-default pull-right\" ng-click=\"vm.navAdd()\">Ajouter un bien</button></div></div><div ng-include=\"'breadcrumbProposer.html'\"></div>"
  );
  ...
  $templateCache.put('/wwwroot/app/search/result/index.html',
    "<div class=\"row\"><div class=\"col-xs-6 col-md-4\"><div ng-repeat=\"object in vm.objects\" ng-class=\"object.class\"><span height=\"120px\"><img ng-src=\"{{object.img}}\" with=\"120px\" height=\"120px\"></span> <span>{{object.texte}}</span> <span>{{object.price}} €</span></div></div><div class=\"col-xs-12 col-md-8\"><div ui-gmap-google-map id=\"map-regie\" center=\"vm.map.center\" zoom=\"vm.map.zoom\" width=\"400px\"><ui-gmap-markers models=\"vm.map.pointList\" coords=\"'position'\" idkey=\"'id'\" click=\"click\" events=\"vm.map.markers2Events\"></ui-gmap-markers></div></div></div>"
  );
 return {}; }]);}());